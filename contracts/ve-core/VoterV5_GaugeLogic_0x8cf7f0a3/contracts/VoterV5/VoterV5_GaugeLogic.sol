...
import {IAlgebraFactoryCustom} from "../interfaces/IAlgebraFactoryCustom.sol";
 ...
 * │  ALM_ALGEBRA_CUSTOM  │     AlgebraFactory       │         GaugeFactoryV2_CL       │
 * └──────────────────────┴──────────────────────────┴─────────────────────────────────┘
 * 
...
enum GaugeType {
    ...
    ALM_ALGEBRA_CUSTOM // 7: Ichi/Gamma concentrated liquidity for a *custom* Algebra pool (NAV/RWA)
}
...
contract VoterV5_GaugeLogic is IVoterV5_GaugeLogic, VoterV5_Storage, ERC165 {
    ...
    /// @notice Custom pool deployer used to resolve ALM_ALGEBRA_CUSTOM pools in the AlgebraFactory
    ///         custom registry (`customPoolByPair`).
    /// @dev Hardcoded to avoid a VoterV5_Storage layout change, mirroring POOL_ELIGIBILITY_ORACLE.
    ///      The Algebra team confirmed this single deployer serves all current and planned Hydrex
    ///      NAV/RWA custom pools. If a *second* custom pool deployer is ever introduced, deploy a
    ///      new VoterV5_GaugeLogic and VoterV5.setGaugeLogic() (or migrate this to a governance-set
    ///      registry). Set is deliberately immutable here.
    address public constant CUSTOM_POOL_DEPLOYER = 0x291A0C1d65101121d130c9E72fDCdd789C47E678;
     ...
     * - 1.8.0: Add ALM_ALGEBRA_CUSTOM gauge type for custom Algebra pools (NAV/RWA), validated
     *          against the factory's customPoolByPair registry using CUSTOM_POOL_DEPLOYER
     */
    string public constant VERSION_GAUGE_LOGIC = "1.8.0";
    ...
    function createGauge(
        ...
        if (_gaugeType == uint256(GaugeType.PAIR_CLASSIC)) {
            ...
            vars.isPair = true;
        } else if (_gaugeType == uint256(GaugeType.ALM_ALGEBRA_CUSTOM)) {
            /**
             * @dev ICHI/GAMMA + Algebra ALM strategy on a *custom* Algebra pool (e.g. NAV/RWA).
             *      Custom pools are absent from `poolByPair`, so the type-1 validation reverts for
             *      them. Instead validate the ALM vault's underlying pool against the factory's
             *      custom registry, keyed by the (immutable) CUSTOM_POOL_DEPLOYER. `dexFactory`
             *      is the same AlgebraFactory configured for type 1.
             */
            (vars.tokenA) = IPairInfo(_pool).token0();
            (vars.tokenB) = IPairInfo(_pool).token1();
            address _pool_factory = IAlgebraFactoryCustom(vars.dexFactory).customPoolByPair(
                CUSTOM_POOL_DEPLOYER,
                vars.tokenA,
                vars.tokenB
            );
            address _pool_hyper = IHypervisor(_pool).pool();
            require(_pool_factory != address(0), "no custom pool");
            require(_pool_hyper == _pool_factory, "wrong tokens");
            vars.isPair = true;
        }

        /// @dev GAUGE_ADMIN can create for any pool, bypassing whitelist/eligibility checks
        bool isGaugeAdmin = IPermissionsRegistry(permissionRegistry).hasRole("GAUGE_ADMIN", msg.sender);
        ...
        /// @dev Enforced for EVERY caller, including the GAUGE_ADMIN whitelist bypass (issue #38):
        ///      derived tokens must be non-zero before they flow unchecked into bribe creation and
        ///      the gauge symbol. The whitelist/eligibility bypass itself remains keyed to GAUGE_ADMIN.
        if (isVaultGauge) {
            require(vars.tokenA != address(0), "!vault.asset");
        } else if (_gaugeType == uint256(GaugeType.ALM_GAMMA_UNISWAP_V4)) {
            // UniswapV4 supports a native token at address(0) for tokenA.
            require(vars.tokenB != address(0), "!tokenB");
        } else {
            require(vars.tokenA != address(0), "!tokenA");
            require(vars.tokenB != address(0), "!tokenB");
        }
